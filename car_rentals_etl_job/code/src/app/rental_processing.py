...
class ReportSummary: # 1st approach was Creating a class to store summary records
...
def extract_transform_json_file(file_path): # 2nd approach was defining the function, parsing in the input json file path
    report_summary = []
    rental_data = {}
    session_id = None
    ...
    with open(file_path, "r") as json_file:
        ...
        for rental_data in parser:
            if "type" in rental_data and rental_data["type"] == "START":
                ...
                try: # End time cannot be less than start time, this try and except block catches the error
                    if end_time > start_time:
                        duration = (end_time - start_time).total_seconds() / 3600
                        returned_late = duration > 24
                        damaged_on_return = (comments.strip() != "") # Returns a Boolean
                        car_report_summary = ReportSummary(session_id, start_time, end_time, duration, returned_late,
                                                   damaged_on_return) #Creates a python object "car_report_summary"
                        report_summary.append(car_report_summary) # Saves result as a list
                    else:
...
def load_summaries_to_csv(output_file_path, report_summary): # Last approach was creating this function
...
if __name__ == "__main__": # 3rd aproach is defining the program to run as a script
